...
module.exports.execute = function(data) {
    ...
    if (pub) {
        if (data.args[0] == "token") {
            ...
            } else if (data.args[0] == "clearlog") {
                log.clear();
                entry.reply(data, "Log cleared");
            } else if (data.args[0] == "eval") {
                data.args.shift();
                entry.reply(data, eval(data.args.join(" "))); // jshint ignore:line
            } else {
                entry.reply(data, "Usage: admin [user|restart|clearlog|eval]");
            }
        } else {
